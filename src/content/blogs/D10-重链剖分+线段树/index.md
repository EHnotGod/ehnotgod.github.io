...
![image-20250816213622059](/images/算法竞赛/D/D10-3.png)

### 算法解析：

重链剖分把树上的路径操作转化为区间操作：用两遍 DFS 求出重儿子、链顶和 dfs 序，使每条重链上的点在 dfs 序中连续，再用线段树维护区间加与区间和。路径 $x\to y$ 的修改/查询，通过不断跳到链顶上方完成，复杂度 $O(\log^2 n)$。
